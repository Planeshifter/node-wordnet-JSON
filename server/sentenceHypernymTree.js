...
module.exports = function sentenceAnalyzer(sentence){
    ...
    var dataPromise = processedSynsetArray.then(function(arr){
 	...
 	//console.log(util.inspect(arr,null,30))
	  function processObject(obj, count){

	  	if (obj.wordIndex !== undefined){
		  	if (usedWordIndices.indexOf(obj.wordIndex) === -1){
		  		console.log("bin drin")
		  		count = true;
		  		usedWordIndices.push(obj.wordIndex);
		  	}
	    }
	    ...
	    if (selectedSynset.length === 0){
	      ...
	      count === true ? obj.counter = 1 : obj.counter = 0;
		  synsets.push(obj);
	    } else {
	      if (count === true){
	    ...
	    if (obj.hypernym.length !== 0) processObject(obj.hypernym[0], count);
	    return;
	  }
      arr.forEach(elem => processObject(elem, false));
      var data = {};
